...
export const RestaurantAssociationSelection = () => {
  ...
  return (
    <div style={style} className="bp3-dark">
      Your restaurants:
      {userRestaurants.map(({ name, key }) => <li key={key}>{name}</li>)}

      <div className="bp3-input-group">
        <span className="bp3-icon bp3-icon-search"/>
        <input className="bp3-input" type="search" placeholder="Search restaurants"
               dir="auto" autoFocus="autoFocus" onKeyPress={onKeyPress}/>
      </div>

      {restaurants.map(({ name, key }) => <li key={key}>{name}</li>)}

      <div style={{ margin: '20px 0' }}>
        <span style={{ marginRight: 5 }}>No matches?</span>
        <Button icon="plus" onClick={() => setShowCreateForm(true)} minimal>Create new restaurant</Button>
      </div>
